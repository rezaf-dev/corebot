        ...
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200">{bot ? 'Edit bot' : 'New bot'}</h2>}>
            ...
            <form onSubmit={submit} className="mx-auto max-w-4xl space-y-4 p-6">
                <div className="rounded-lg bg-white p-5 shadow dark:bg-gray-800">
                    ...
                    <label className="mt-4 block text-sm text-gray-700 dark:text-gray-300">System prompt<textarea value={data.system_prompt} onChange={(e) => setData('system_prompt', e.target.value)} className="mt-1 h-36 w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" /></label>
                    <label className="mt-4 block text-sm text-gray-700 dark:text-gray-300">Allowed domains<textarea value={data.allowed_domains} onChange={(e) => setData('allowed_domains', e.target.value)} className="mt-1 h-20 w-full rounded border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100" /></label>
                    ...
                    <label className="mt-4 block text-sm text-gray-700 dark:text-gray-300"><input type="checkbox" checked={data.collect_visitor_email} onChange={(e) => setData('collect_visitor_email', e.target.checked)} /> Collect email</label>
                    <label className="mt-2 block text-sm text-gray-700 dark:text-gray-300"><input type="checkbox" checked={data.collect_visitor_phone} onChange={(e) => setData('collect_visitor_phone', e.target.checked)} /> Collect phone</label>
